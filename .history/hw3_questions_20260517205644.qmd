...
The simple Poisson model assumes that every firm shares the same expected patent rate. This assumption is unrealistic because patent outcomes are likely influenced by firm-specific characteristics such as age, geographic region, and Blueprinty customer status.

To allow patent rates to vary across firms, I extended the model to a Poisson regression framework:
...
The exponential link function is used because the Poisson rate parameter \(\lambda_i\) must always be positive, while the linear predictor \(X_i'\beta\) can take any real value. Applying the exponential transformation guarantees positive predicted patent rates.
...
- and a binary indicator for Blueprinty customer status.

Age squared was included because the relationship between firm age and patent production is unlikely to be perfectly linear. One regional indicator was omitted to avoid perfect multicollinearity with the intercept.
...
Next, I estimated the coefficient vector \(\hat{\beta}\) numerically using Maximum Likelihood Estimation. I also requested the Hessian matrix from the optimizer in order to compute standard errors.
...
To validate the results, I estimated the same model using Python’s built-in Poisson GLM implementation.
...
The coefficient estimates from the manual MLE procedure and the built-in GLM implementation are very similar, confirming that the likelihood function and optimization procedure were implemented correctly. Small differences in the standard errors arise because the Hessian matrix from numerical optimization is only an approximation.

The regression results indicate that Blueprinty customer status is positively associated with patent counts even after controlling for age and regional differences. The customer coefficient is positive, suggesting that firms using Blueprinty’s software are expected to receive more patents on average than otherwise similar firms that do not use the software.

The coefficients on age and age squared imply a nonlinear relationship between firm age and patenting activity. Patent production tends to increase with age initially, although the effect gradually weakens for older firms.

Regional coefficients capture geographic differences in innovation intensity and patent activity. Some regions appear to produce systematically more patents than others even after controlling for firm characteristics.
