...
The MaxDiff data can be converted into multinomial logit choice observations. Each task provides two choices. First, the respondent chooses the best item from the full set of four. Second, after the best item is removed, the respondent chooses the worst item from the remaining three.

The MNL model assigns each item a utility coefficient $\beta_j$. If the task shows items $\{a,b,c,d\}$, the probability that item $b$ is selected as best is:
...
The probability that item $c$ is selected as worst after item $b$ is removed is modeled by flipping utilities:
...
Because adding the same constant to every utility does not change choice probabilities, one coefficient must be fixed. I set $\beta_1 = 0$ and estimate $\beta_2$ through $\beta_{10}$.
...
print(task_check[["n_rows", "n_best", "n_worst", "n_neutral"]].value_counts())
...
data_valid = data.merge(valid_tasks, on=["record_id", "task"], how="inner")

print("Original rows:", len(data))
print("Valid rows:", len(data_valid))
print("Valid tasks:", len(valid_tasks))

# Build task_data from valid tasks only
task_data = (
